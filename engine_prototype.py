...
def fill_sentence_engine(sent_df, model="claude-sonnet-4-6",
    ...
    Chunks scripts into at most _SENT_CHUNK sentences per call so the
    response never truncates. Scales max_tokens to chunk size. Retries
    once on failure; leaves a visible FAILED(engine) marker on a second
    ...
    for ident, g in df.groupby("Identifier", sort=False):
        ...
        if mock:
            print(f"MOCK script {ident}: {len(items)} sentences would be "
                  f"sent in chunks of {_SENT_CHUNK} with full-script context.")
            print("first item:", json.dumps(items[0]))
            print("system prompt head:\n", _SENT_SYSTEM[:240], "...\n")
            return df

        # chunk so the JSON response always fits within max_tokens
        for start in range(0, len(items), _SENT_CHUNK):
            chunk_items = items[start:start + _SENT_CHUNK]
            chunk_refs  = refs[start:start + _SENT_CHUNK]
            max_tok = min(len(chunk_items) * _SENT_TOK_PER + _SENT_TOK_OVERHEAD,
                          8192)
            tags = {}
            for attempt in range(2):
                try:
                    ...
                    txt = re.sub(r"```json|```", "", txt).strip()
                    tags = {t["id"]: t for t in json.loads(txt)}
                    break
                except Exception:
                    if attempt == 0:
                        continue   # retry once
                    # second failure: mark visibly so it is never mistaken
                    # for "not run"
                    for _, ref in chunk_refs:
                        out_type[ref] = "FAILED(engine)"
            for n, ref in chunk_refs:
                if out_type.get(ref) == "FAILED(engine)":
                    continue
                t = tags.get(n, {})
                ...
                if pv in ("Correct", "Incorrect", "NA"):
                    out_pv[ref] = pv
                    out_ps[ref] = (t.get("pronoun_subtype", "")
                                   if pv == "Incorrect" else "NA")

    if not mock:
        df["Sentence type"] = df.apply(
            lambda r: out_type.get(r["SentenceRef"], r["Sentence type"]),
            axis=1)
        df["Pronoun ref"] = df.apply(
            lambda r: out_pv.get(r["SentenceRef"], r["Pronoun ref"]), axis=1)
        df["Pronoun ref subtype"] = df.apply(
            lambda r: out_ps.get(r["SentenceRef"], r["Pronoun ref subtype"]),
            axis=1)
    return df
